test_that("runInNewRSession works properly", {
  ...
  expect_identical(tail(runInNewRSession(function() 1 + 1, workFilePath = workFile, stdout = TRUE, stderr = TRUE,
                                         cleanupWorkFile = FALSE), 1),
                   "[1] 2")
  expect_true(file.exists(workFile))
  expect_identical(tail(runInNewRSession(function(x) 1 + x, list(x = 1), workFilePath = workFile, stdout = TRUE,
...
test_that("runInNewRSession detects malformed input", {
  ...
  expect_error(runInNewRSession(function() 0, workFilePath = "./nonexistentpath/bla.rds"),
               "file.create(workFilePath, showWarnings = FALSE) is not TRUE", fixed = TRUE)
  ...
  expect_error(runInNewRSession(function() 0, workFilePath = workFile, unknownArgument = NULL),
